...
def load_data(dataname, batch_size, val_batch_size, num_workers, data_root, dist=False, **kwargs):
    ...
    if dataname == 'bair':
        ...
        from .dataloader_sc import load_data
        return load_data(batch_size, val_batch_size, data_root, num_workers, **cfg_dataloader)
    elif dataname == 'sc_128':
        from .dataloader_sc_128 import load_data
        return load_data(batch_size, val_batch_size, data_root, num_workers, **cfg_dataloader)
    elif dataname == 'hko7':
        from .dataloader_hko7 import load_data
        return load_data(128, batch_size, val_batch_size, num_workers=num_workers)
    elif dataname == 'imerg':
        from .dataloader_imerg import load_data
        return load_data(batch_size, val_batch_size, num_workers=num_workers)
